...
defineOptions({
    layout: {
        breadcrumbs: [
            ...
            { title: 'Edit Transaksi', href: '/transactions/edit' },
        ],
    },
});
...
const form = useForm({
    financial_account_id: String(props.transaction.financial_account_id),
...
const hasExistingAttachment = computed(() => {
    return Boolean(props.transaction.attachment && !removeExistingAttachment.value);
});
...
const handleFileChange = (event: Event) => {
    ...
    removeExistingAttachment.value = true;
...
const removeNewAttachment = () => {
    ...
    if (!props.transaction.attachment) {
...
const removeOldAttachment = () => {
    removeExistingAttachment.value = true;
    form.remove_attachment = true;
};

const restoreOldAttachment = () => {
    removeExistingAttachment.value = false;
    form.remove_attachment = false;
    attachmentFile.value = null;
    attachmentPreview.value = null;
...
const submit = () => {
    form.post(`/transactions/${props.transaction.id}`, {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
<template>
    <Head title="Edit Transaksi - Loovie Apps" />
                    ...
                    <div>
                        ...
                        >
                            <div class="flex items-start justify-between gap-3">
                                ...
                                <button
                                    type="button"
                                    class="rounded-lg p-1 text-red-600 hover:bg-red-100 hover:text-red-700 dark:hover:bg-red-950"
                                    @click="removeOldAttachment"
                                >
                                    <X class="h-5 w-5" />
                                </button>
                            </div>
                        ...
                        <div
                            v-if="removeExistingAttachment && props.transaction.attachment && !attachmentFile"
                            class="mb-4 rounded-xl border border-yellow-200 bg-yellow-50 p-4 text-sm text-yellow-700 dark:border-yellow-900 dark:bg-yellow-950/40 dark:text-yellow-200"
                        >
                            Lampiran lama akan dihapus saat perubahan disimpan.

                            <button
                                type="button"
                                class="ml-2 font-semibold underline"
                                @click="restoreOldAttachment"
                            >
                                Batalkan hapus
                            </button>
                        </div>
                        ...
                        >
                            ...
                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                Klik untuk upload lampiran baru
                            </p>
            ...
            <div class="space-y-6 lg:col-span-1">
                ...
                >
                    ...
                    <ul class="mt-3 space-y-2 text-sm text-gray-600 dark:text-gray-400">
                        <li>• Perubahan transaksi dapat memengaruhi saldo akun.</li>
                        <li>• Mengganti tipe akan memfilter kategori sesuai tipe baru.</li>
                        <li>• Upload lampiran baru akan mengganti lampiran lama.</li>
                    </ul>
                </div>
            </div>
        </div>
